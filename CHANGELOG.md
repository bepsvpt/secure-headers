...
- 6.0.0 (unreleased)
  - **BREAKING CHANGE**
    - Lumen does not automatically add SecureHeadersMiddleware to global middleware.
    ...
    - Content-Security-Policy empty directive config will not add to response header(before 6.0 was `'none'`). 
    ...
    - `SecureHeaders::nonce` method requires parameter now, it should be `'script'` or `'style'`(default is `'script'`).
